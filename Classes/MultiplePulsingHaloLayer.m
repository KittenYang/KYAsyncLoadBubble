...
-(void)stopPulse{
    ...
    CABasicAnimation *stopAnimation = [CABasicAnimation animationWithKeyPath:@"transform.scale.xy"];
    stopAnimation.toValue = @(0);
    stopAnimation.duration = 0.3;
    [self addAnimation:stopAnimation forKey:@"stopAnimation"];
    
    [self performSelector:@selector(clearLayer) withObject:nil afterDelay:0.2];
